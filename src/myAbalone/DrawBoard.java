...
public class DrawBoard extends JPanel {
  ...
  public void paintComponent(Graphics g) {
	...
	g.setColor(Color.yellow);
     ...
     for (int i = 0; i < 6; i++){
      background.addPoint((int) (420 + 400 * Math.cos(i * 2 * Math.PI / 6)),
    ...
    for(int i=4;i>=0;i--){
        for(int j=0;i+j<9;j++) {
            ...
            if (value==1) color= new Color(0,0,0);
            if (value==-1) color = new Color(255,255,255);
            g2.setColor(color);
            g2.fillOval(j*84+40+45*(i), 80*i+310, 40, 40);
            g2.drawString(Integer.toString(value),j*84+40+45*(i), 80*i+310);
        }
      }
    for(int j=1;j<5;j++){
	    for(int i=0;i<9-j;i++) {
            ...
            g2.drawString(Integer.toString(value),i*84+40+45*(j), 310-80*j);
			}
