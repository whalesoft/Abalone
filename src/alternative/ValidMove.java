public class ValidMove
{
	...
	// --- else move the marbles in all positions - ADD an opponent marble in the
	// specified place &
	...
	// test valid moves
	public int validMove ( int player, int board[][], int x[], int y[],
			int movement )
	{
		...
		{ // UPLEFT
			...
			if ( x [ 2 ] != -1
					&& ( board [ x [ 2 ] ] [ y [ 2 ] ] == emptySpace || board [ x [ 2 ] - 1 ] [ y [ 2 ] - 1 ] != player ) )
				return 0;
			if ( x [ 1 ] != -1
					&& ( board [ x [ 1 ] ] [ y [ 1 ] ] == emptySpace || board [ x [ 1 ] - 1 ] [ y [ 1 ] - 1 ] != player ) )
				return 0;
			if ( board [ x [ 0 ] ] [ y [ 0 ] ] == emptySpace
