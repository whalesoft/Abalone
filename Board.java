	...
	public void setPos(int i, int a, int b) throws InvalidMove {
		...
		if (pos[i][2] == 2)
			if (board.get(a, b) != -1)
				...
				throw new InvalidMove();// System.out.println("Not a valid move!");
		...
		for (int p = 0; p < c; p++) {
			...
			if (s < 2) {
				if (c > counter) {
					...
					try {
						black.move(c * min[0] + i, c * min[1] + j);
					} catch (InvalidMove e) {
					}
					try {
						move(i, j);
...
public class Board {
	...
	public static void main(String[] args) {
		...
		test.select(4, 6);
		for (int i = 0; i < 14; i++)
			if (test.getPos(i)[2] == 2)
				System.out.println(test.getPos(i)[0] + " " + test.getPos(i)[1]
						+ " " + i + " Selected");
		/*
		 * test.setPos(4,4,2); test.setPos(7,4,4); test.setPos(10,5,5);
		 */
